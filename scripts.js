 let scene, camera, renderer, pen;
    ...
    const texture = new THREE.TextureLoader().load(imageData, function(tex) {
        ...
        for (let i = 0; i < uv.count; i++) {
            ...
            // Adjust to prevent seam
            const adjustedU = (u + 0.5) % 1.0;
    ...
    window.addEventListener('load', function() {
            function loadCCapture() {
                return new Promise((resolve, reject) => {
                    if (typeof CCapture !== 'undefined') {
                        console.log('CCapture is already loaded.');
                        resolve();
                    } else {
                        console.log('Attempting to load CCapture manually.');
                        var script = document.createElement('script');
                        script.src = 'https://cdnjs.cloudflare.com/ajax/libs/CCapture.js/1.1.0/CCapture.all.min.js';
                        ...
                        script.onerror = function() {
                            ...
                            reject(new Error('Failed to load CCapture'));
                        };
                        document.body.appendChild(script);
                    }
                });
            ...
            loadCCapture().then(() => {
                console.log('CCapture is ready to use.');
            }).catch((error) => {
                console.error('Error loading CCapture:', error);
            });
        });
</script>
<script>
